        ...
        if "Scores" in form.scores_scorers.data:
            ...
            if int(form.select_one.data) >= 2:
                if int(form.select_one.data) >= 3:
                    ...
                    third_lowest_comments = get_all_nth_lowest_comments_scorer(2, start_length, all_managers, latest_gw, 2)

                    all_third_lowest = get_all_nth_lowest_scorer(2, start_length, all_managers, latest_gw, 2)

                    times_third_last = list_to_dict(all_third_lowest)

                    amount_owed_third_last = get_amount_owed(times_third_last, form.amount_owed_3.data)

                    second_lowest_comments = get_all_nth_lowest_comments_scorer(2, start_length, all_managers,
                                                                                latest_gw, 1)

                    all_second_lowest = get_all_nth_lowest_scorer(2, start_length, all_managers, latest_gw, 1)

                    times_second_last = list_to_dict(all_second_lowest)

                    amount_owed_second_last = get_amount_owed(times_second_last, form.amount_owed_2.data)
                else:
                    second_lowest_comments = get_all_nth_lowest_comments_scorer(1, start_length, all_managers,
                                                                                latest_gw, 1)

                    all_second_lowest = get_all_nth_lowest_scorer(1, start_length, all_managers, latest_gw, 1)
